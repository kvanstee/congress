  ...
  start: async (account,_provider) => {
    ...
    congress.members(account).then(async res => {
          ...
          const filter = congress.filters.LogProposalTallied(ID);
          	...
          	if (!Ids.has(ID)) {
          		Ids.add(ID);
							App.write_proposal(account,ID,min_quor,maj_mar);
						}
					})
				})
			})
			...
			congress.on('LogChangeOfRules', (NewMinimumQuorum, NewMajorityMargin) => {
			...
			document.getElementById("right").onclick = () => {
				...
				["add_member","remove_member","change_voting_rules"].forEach(action => {
					document.getElementById(action).onclick = () => {
					  congress.newProposal(congress_addr,0,action, App.get_bytecode(action)).catch(err => alert(err));
					}
				})
			}
		})
	...
	//END OF App.start()
	//WATCH FOR VOTE
	watch_for_vote: (account,ID,min_quor,maj_mar) => {
		const filter = congress.filters.LogVoted(ID);
	  congress.on(filter, (id, vote, voter) => {
	    const proposal_elements = document.getElementById(ID).getElementsByTagName("td");
	    congress.proposals(id).then(proposal => {
				const num_votes = Number(proposal[6]);
				const cum_vote = Number(proposal[7]);
				proposal_elements[4].innerHTML = num_votes; //number of votes
				proposal_elements[5].innerHTML = cum_vote;  //cumulative vote
				if (num_votes >= min_quor) proposal_elements[4].style.color = "green";
				if (cum_vote >= maj_mar) proposal_elements[5].style.color = "green";
		  	if (num_votes >= min_quor && cum_vote >= maj_mar && Date.now()/1e3 > proposal[3]) {
	...
	//WRITE PROPOSAL
  write_proposal: async (account,ID,min_quor,maj_mar) => {
    ...
    if (proposal[4]) return;
	  ...
	  _proposal.id = ID;
	  const proposal_elements = _proposal.getElementsByTagName('td');
    ...
    proposal_elements[5].innerHTML = cum_vote;
	  ...
	  (cum_vote >= maj_mar) ? proposal_elements[5].style.color="green" : proposal_elements[5].style.color="red";
		if (num_votes < min_quor || cum_vote < maj_mar) {
			App.watch_for_vote(account,ID,min_quor,maj_mar)
			//SEE IF YOU VOTED
			const filter = congress.filters.LogVoted(ID, null, account); //indexed variables
